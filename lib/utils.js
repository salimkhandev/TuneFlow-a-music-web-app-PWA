...
// Deduplicate an array of songs based on title and primary artists
export const deduplicateSongs = (songs) => {
...
// Fetch Artists
export async function fetchArtists({ query = "top artists", page = 1, limit = 10 }) {
  const url = `${API_BASE}/api/search/artists?query=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
  return await fetchWithCache(url, `artists_${query}_${page}_${limit}`);
}
  ...
  try {
    ...
    if (data?.data) {
      if (data.data.topSongs) {
        data.data.topSongs = deduplicateSongs(data.data.topSongs);
      }
...
// Fetch Songs
export async function fetchSongs({ query = "trending songs", page = 1, limit = 10 }) {
  const url = `${API_BASE}/api/search/songs?query=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
  const data = await fetchWithCache(url, `songs_${query}_${page}_${limit}`);
  if (data?.data?.results) {
    data.data.results = deduplicateSongs(data.data.results);
  }
  return data;
}

// Fetch Albums
export async function fetchAlbums({ query = "new albums", page = 1, limit = 10 }) {
  const url = `${API_BASE}/api/search/albums?query=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
  return await fetchWithCache(url, `albums_${query}_${page}_${limit}`);
}
...
export async function fetchAlbumById({ id }) {
  ...
  try {
    ...
    if (data?.data) {
      if (data.data.songs) {
        data.data.songs = deduplicateSongs(data.data.songs);
      }
      await cacheResponse(cacheKey, data.data);
      return data.data;
    }
